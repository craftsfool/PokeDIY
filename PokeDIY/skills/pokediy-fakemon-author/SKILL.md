...
4. Search the project's local datasets before naming an official move, ability, species, form, or evolution method. Prefer exact local IDs and Simplified Chinese names. In the PokeDIY workspace, search `../data/reference/original-content.json`, `../data/reference/evolution-methods.json`, `../data/national/national-pokedex.json`, and `../data/national/pokemon-forms.json` with `rg` or a JSON-aware tool.
...
7. Run `python scripts/validate_pokediy.py <draft.json>`. Fix every error. Review warnings deliberately rather than suppressing them.
8. For artwork, read [references/art-direction.md](references/art-direction.md), then run `python scripts/build_art_prompt.py <draft.json> --out <prompt.txt>` to produce the locked visual recipe.
